...
export async function generateJWT(
  ...
  try {
    ...
    const cookieOptions: any = {
      ...
      maxAge: 86400, // an alternative: Indicates the number of seconds until the Cookie expires.
    ...
    // the '.setHeader' method does not accept new lines as an argument, so we have to get rid of it:
    // cutting litle dinosour forms
    cookie = cookie.replace(/\s/g, ' ')
    console.log('The Cookie fresh out of the Oven:', cookie)

    response.setHeader(`Set-Cookie`, cookie)

    // If you prefer to use Max-Age instead:
    // ${
